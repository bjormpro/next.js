...
/// Whether a [`ExecuteContext::task`] call may create the task or requires it to already exist.
///
/// Every `task`/`task_pair` call site chooses one, so the intent — "I am opening a task I know
/// exists" vs "I am creating or looking-up-or-creating a task" — is explicit and greppable rather
/// than implicit in the default behavior.
...
pub enum TaskAccess {
    ...
    /// Use for genuine creation (`get_or_create_task`, connecting a new child) and for edge-follows
    /// where the target may be freshly created (aggregation followers/uppers).
    MaybeCreate,
...
pub trait ExecuteContext<'e>: Sized {
    ...
    where
        'e: 'l;
    /// Opens a task, restoring the requested `category` from persistent storage if needed.
    /// `access` declares whether the task may be created ([`TaskAccess::MaybeCreate`]) or must
    /// already exist ([`TaskAccess::MustExist`]).
    fn task(
        &mut self,
        task_id: TaskId,
    ...
    /// Opens two tasks under a single lock acquisition (to atomically read/mutate an edge between
    /// them). `access` applies to both ids.
    fn task_pair(
        &mut self,
        task_id1: TaskId,
        task_id2: TaskId,
    ...
    fn open(
        ...
        if !task.flags.is_restored(category) {
            if task_id.is_transient() {
                ...
                assert!(
                    !maybe_fabricated,
                    "task({task_id}, MustExist): transient task is not resident — it does not \
                     exist"
                );
            } else {
        ...
        // A `MustExist` open of a GC-soft-deleted task means a resurrection path was missed: the
        // task was collected (edges scrubbed), so reading it would return stale contents. Every
        // re-entry is supposed to funnel through `resurrect_deleted` at connect first. Debug-only;
        // the flag only exists when GC is enabled.
        #[cfg(debug_assertions)]
        if access == TaskAccess::MustExist {
            debug_assert!(
                !task.flags.deleted(),
                "task({task_id}, MustExist) opened a GC-deleted task — a resurrection path was \
                 missed"
            );
        }
        TaskGuardImpl {
...
impl<'e> ExecuteContext<'e> for ExecuteContextImpl<'e> {
    ...
    fn task(
        ...
        self.open(task_id, category, access)
    }
    ...
    fn task_pair(
        ...
        // A `MustExist` pair open must not target GC-soft-deleted tasks: both endpoints of the edge
        // being read/rebalanced must be live (a `deleted` one means a resurrection path was
        // missed). Debug-only; the flag only exists when GC is enabled. (Full
        // non-fabrication for the pair path — like `open`'s single-task existence check —
        // is a follow-up; the current callers hold live references to both tasks.)
        #[cfg(debug_assertions)]
        if access == TaskAccess::MustExist {
            debug_assert!(
                !task1.flags.deleted(),
                "task_pair({task_id1}, .., MustExist) opened a GC-deleted task — a resurrection \
                 path was missed"
            );
            debug_assert!(
                !task2.flags.deleted(),
                "task_pair(.., {task_id2}, MustExist) opened a GC-deleted task — a resurrection \
                 path was missed"
            );
        }
